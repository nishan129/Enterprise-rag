    ...
    with logfire.span("📄 Document AI Parsing", filename=file_path):
        try:
            ...
            if total_pages <= MAX_PAGES_PER_REQUEST:
                ...
                full_text = process_document_chunk(content=image_content, name=name)

            else:
                # Split into chunks of MAX_PAGES_PER_REQUEST
                logfire.info(f"PDF exceed {MAX_PAGES_PER_REQUEST} pages. Splitting into chunks.")
                ...
                for i in range(0, total_pages, MAX_PAGES_PER_REQUEST):
                    ...
                    with logfire.span(f"Processing pages {i+1} to {chunk_end}"):
                        chunk_text = process_document_chunk(content=chunk_bytes, name=name)
                        full_text += chunk_text + "\n"
            if not full_text.strip():
                logfire.warning("⚠️ Document AI returned empty text for {file_path}")
            else:
                logfire.info(f"✅ Document AI succesfully parsed {len(full_text)} characters")
            
            return full_text
        except Exception as e:
            logfire.error(f"❌ Error processing PDF with Document AI: {str(e)}")
            return ""

def process_document_chunk(content: bytes, name:str) -> str:
    """
